...
def generate_report(table_name, column_name, filter_value=None, filename='report.csv'):
    """
    Generuje raport na podstawie danych z bazy danych.

    Args:
        table_name (str): Nazwa tabeli.
        column_name (str): Nazwa kolumny do filtrowania.
        filter_value (str, optional): Wartość filtru dla kolumny (domyślnie: None).
        filename (str, optional): Nazwa pliku raportu (domyślnie: 'report.csv').

    W zależności od podanych argumentów, generuje raport na podstawie danych z tabeli
    i zapisuje go jako plik CSV w folderze "results". Jeśli tabela lub kolumna nie istnieje,
    lub jeśli brak wyników filtrowania, zostaje wyświetlony odpowiedni komunikat.
    """
