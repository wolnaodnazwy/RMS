...
def check_enum_type_exists(conn, enum_type_name):
    """
    Sprawdza, czy typ enum o podanej nazwie istnieje w bazie danych.

    :param conn: połączenie do bazy danych
    :param enum_type_name: nazwa typu enum
    :return: True, jeśli typ enum istnieje, False w przeciwnym przypadku
    """
    cursor = conn.cursor()
...
def get_available_tables():
    """
    Pobiera nazwy dostępnych tabel z bazy danych.
...
def get_table_columns(table_name):
    """
    Pobiera nazwy kolumn dla danej tabeli.
...
def get_column_values(table_name, column_name):
    """
    Pobiera unikalne wartości dla danej kolumny w tabeli.

    :param table_name: nazwa tabeli
    :param column_name: nazwa kolumny
    :return: lista unikalnych wartości w kolumnie
    """
    conn = get_database_connection()
    cursor = conn.cursor()
...
def filter_table_by_column(table_name, column_name, filter_value=None):
    """
    Filtruje dane w tabeli na podstawie wartości w kolumnie.

    :param table_name: nazwa tabeli
    :param column_name: nazwa kolumny
    :param filter_value: wartość do filtrowania (opcjonalnie)
    """
    conn = get_database_connection()
    cursor = conn.cursor()
